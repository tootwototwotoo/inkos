...
function buildShortContinuePrompt(storyId: string, isZh: boolean): string {
  return isZh
    ? `你是 InkOS 短篇继续编辑助手。当前会话已绑定短篇「${storyId}」，产物在 shorts/${storyId}/。
...
- final/full.md：完整正文（全章节合并）。改单章后若要保持同步，需同步更新此文件。
- final/chapters/0001.md、0002.md…：按章拆分的正文，文件名是 4 位数字，无标题后缀。
- final/short-story.json：结构化元数据（storyTitle、chapters[].title/content、language）。
- final/sales-package.md：简介、卖点、封面提示词。
- final/cover-prompt.md：封面提示词。
- final/cover.png / cover.jpg：封面图（可选）。
- outline/v002.md、reviews/：大纲和审稿记录，一般不改。
...
- 局部定点修改（改一段对话、一处描写）-> edit_short_file，oldString 带足够上下文确保唯一。
- 整章重写或整篇重排 -> write_short_file。改完 final/chapters/NNNN.md 后，如果 final/full.md 需要保持一致，用 write_short_file 或 edit_short_file 同步更新。
- 改简介/卖点/封面提示词 -> edit_short_file 或 write_short_file 改 final/sales-package.md / final/cover-prompt.md。
- 重做封面 -> generate_cover。
- 讨论性问题（不动文件）-> 直接回答。
...
- final/full.md: complete manuscript (all chapters merged). Keep it in sync after editing a chapter.
- final/chapters/0001.md, 0002.md...: per-chapter files named as 4-digit numbers, no title suffix.
- final/short-story.json: structured metadata (storyTitle, chapters[].title/content, language).
- final/sales-package.md: synopsis, selling points, cover prompt.
- final/cover-prompt.md: cover prompt.
- final/cover.png / cover.jpg: cover image (optional).
- outline/v002.md, reviews/: outline and review records; generally do not edit.
...
- Local targeted edit (a line of dialogue, a description) -> edit_short_file with enough context in oldString to be unique.
- Whole-chapter rewrite or full-manuscript reflow -> write_short_file. After editing final/chapters/NNNN.md, sync final/full.md if it should stay consistent.
- Edit synopsis/selling points/cover prompt -> edit_short_file or write_short_file on final/sales-package.md / final/cover-prompt.md.
  ...
  if (sessionKind === "short") {
    const confirmedIntent = isConfirmedAction(options, "short_run")
      ...
      : isConfirmedAction(options, "generate_cover")
        ? "generate_cover"
        : undefined;
    // 已绑定 storyId(bookId 非空)且非 confirmed 生产意图:继续编辑会话。
    if (!confirmedIntent && bookId) {
      return withSkills(buildShortContinuePrompt(bookId, isZh));
    }
